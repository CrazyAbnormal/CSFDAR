...
const emit = defineEmits(["reloadSections", "input"]);
...
const saveSections = async () => {
    if (action_clicked.value == "Add") {
        router.post("/sections/add", form);
    } else if (action_clicked.value == "Update") {
        router.post("/sections/update", form);
    }
    ...
    <v-dialog v-model="show_form_modal" width="600" scrollable persistent>
        <v-card>
            <v-card-title class="bg-indigo">
                <span class="text-h5">{{ props.action }} Region</span>
            </v-card-title>
            <v-card-text>
